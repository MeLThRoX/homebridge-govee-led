...
class Govee {
    ...
    configureAccessory(accessory) {
        ...
        service.getCharacteristic(Characteristic.Brightness)
            .on("get", (callback) => {
                this.govee.state(accessory.context.device).then(dev_res => {
                    let brightness = Math.round(dev_res.properties.find(prop => prop.brightness).brightness / 255 * 100)
        ...
        service.getCharacteristic(Characteristic.Hue)
            .on('get', (callback) => {
                this.govee.state(accessory.context.device).then(dev_res => {
                    this.log("Hue - RGB: " + dev_res.properties.find(prop => prop.color).color)
                    let hsv = convert.rgb.hsv(dev_res.properties.find(prop => prop.color).color)
                    this.log("Hue - HSV: " + hsv)
                    callback(null, hsv[0])
                }).catch(reason => {
                    if (this.debug) this.log.error(reason)
                })
            ...
            .on('set', (value, callback) => {
                ...
                let rgb = convert.hsv.rgb(value, saturation, 100)
        ...
        service.getCharacteristic(Characteristic.Saturation)
            ...
            .on('set', (value, callback) => {
                ...
                let rgb = convert.hsv.rgb(hue, value, 100)
