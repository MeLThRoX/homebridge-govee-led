...
class Govee {
            ...
            let accs = govee_data.data.devices.map(govee_device => {
                ...
                if (!this.accessories.find(accessory => accessory.UUID === govee_device.uuid)) {
                    ...
                    acc.addService(Service.Lightbulb)
                        .on('get', this.handleOnGet.bind(this, govee_device))
                        .on('set', this.handleOnSet.bind(this, govee_device))
                    acc.context = {
    ...
    handleOnGet(callback) {
        this.log(govee_device)
        this.govee.state(govee_device.device).then(dev_res => {
            callback(null, dev_res.properties.find(prop => prop.powerState).powerState === 'on' ? true : false)
        })
    }

    handleOnSet(value, callback) {
        this.log(govee_device)
        govee.control(govee_device.device, {
            name: 'turn',
            value: value ? 'on' : 'off'
        }).then(dev_res => {
            callback(null)
