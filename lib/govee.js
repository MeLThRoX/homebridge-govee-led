    ...
    syncDevices() {
        this.govee.get().then(govee_data => {
            ...
            accs = this.accessories.map(accessory => {
                if (!govee_data.data.devices.find(govee_device => govee_device.device === accessory.context.device)) {
    ...
    configureAccessory(accessory) {
        ...
        service.getCharacteristic(Characteristic.On)
            .on("get", (callback) => { // get device powerState
                this.govee.state(accessory.context.device, accessory.context.model).then(dev_res => {
            ...
            .on("set", (value, callback) => { // set device powerState
                this.govee.control(accessory.context.device, accessory.context.model, {
                    name: 'turn',
                    value: value ? 'on' : 'off'
                }).then(dev_res => {
                    callback(null)
